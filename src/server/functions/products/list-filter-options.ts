...
export const listProductFilterOptions = createServerFn({
	method: "GET",
}).handler(async () => {
	...
	return {
		...
		categories: (categoryRows.rows as Array<{ category: string }>).map(
			(r) => r.category,
		),
		sourceCountries: (
			countryRows.rows as Array<{ source_country: string }>
		).map((r) => r.source_country),
	};
});
